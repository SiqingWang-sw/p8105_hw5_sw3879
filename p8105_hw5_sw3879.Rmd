...
When true μ = 0 or when true μ is larger (> 4), we can see that the average estimate of μ_hat when null is rejected is equal to the true μ. When the true μ = 0, we are testing this against the H0 μ = 0, so both estimates will be close. 

When true μ is 1 - 3, this deviation starts out large and gradually becomes smaller. This is because while estimates of the fail-to-reject-null group is close to 0, estimates of the rejected groups will be close to the true μ, and as true μ increases, more and more reps out of the 5000 gets rejected, and the estimate from the rejected group will tend toward the true μ.

By when true μ is very large, basically all reps out of the 5000 reps get rejected, so the estimate from the rejected group would again be very similar to the true μ. 
